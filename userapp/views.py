...
def userConfiguration(request):
    ...
    if request.method == "POST":
        ...
        if request.FILES:
            rpta = DefaultUser.updateUser(idAuth, firstName, lastName, address, country, phone, request.FILES['avatar'])
            author = idAuth
            action = 'ha actualizado su foto de perfil'
            content = ''
            if rpta == 1: Post.addPost(author,action,content,request.FILES['avatar'])
        else:
